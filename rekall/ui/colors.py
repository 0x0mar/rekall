...
def EulersNumber(precision=100, iterations=150):
    """Computes the Euler's to arbitrary precision."""
    factorial = 1
    euler = 2
    ...
    for x in xrange(2, iterations):
        factorial *= x
        euler += one / decimal.Decimal(factorial)
...
def RGBToXTerm(red, green, blue):
    ...
    return (16 # base offset
            ...
            + ChannelStepFunction(red) * 6**2) # Red increases in an outer loop.
